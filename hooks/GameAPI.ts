...
export const fetchGameData = async (type: GameType, lines: string[], language: string) => {
  ...
  // 1. HANDLE LOCAL GAMES FIRST (Speech)
  // We check this first so we don't need a backend endpoint or fetch call
  if (type === "speech") {
    // Pick a random line from the simplifiedLines
    const randomLine = lines.length > 0 
      ? lines[Math.floor(Math.random() * lines.length)] 
      : "Hello World"; // Fallback if empty
  ...
  // 2. SETUP ENDPOINTS FOR BACKEND GAMES
  let endpoint = "";
  ...
  switch (type) {
    ...
    case "truefalse": endpoint = "/generate_tf"; break;
    case "missing": endpoint = "/generate_missing_letters"; break;
    default: return null; // If type is unknown (and not speech), stop here.
  }
